class FlushValidator():
    ...
    def valid_cards(self):
        cards = [card for card in self.cards if card.suit in self._suits_that_occur_five_or_more_times.keys()]
        return cards[-5:]

    @property
    def _suits_that_occur_five_or_more_times(self):
        suits_that_occur_five_or_more_times = {
            suit:suit_count for suit, suit_count in self._card_suit_counts.items()
            if suit_count >= 5
        }
        return suits_that_occur_five_or_more_times
